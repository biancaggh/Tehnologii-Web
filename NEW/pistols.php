<?php
    ...
    if(isset($_POST['search'])){
        ...
        $query = mysqli_query($con,"SELECT * FROM pistols WHERE title LIKE '%$searchq%' OR type LIKE '%$searchq%' OR place LIKE '%$searchq%' OR calibre LIKE '%$searchq%' OR cartridge LIKE '%$searchq%' OR barrel LIKE '%$searchq%' OR ef_range LIKE '%$searchq%' OR feed LIKE '%$searchq%' OR material LIKE '%$searchq%'"); 
...
<html lang="en">
    <head>
        <title> Terrorism | Pistols </title>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link href="pistols.css" rel="stylesheet" type="text/css">
    ...
    <body>
        ...
        <div class="search-box">
            <form action="pistols.php" method="post">
            ...
            <div class="container">
                <?php
                    $query = "SELECT * FROM pistols ORDER BY id ASC ";
                    ...
                    if(mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            <div class="weapon">
                                ...
                                <form method="post" action="pistols.php?action=add&id=<?php echo $row["id"]; ?>">
                                <?php
                                ?>
                                    <div class="pistol">
                                        <img src="<?php echo $row["image"]; ?>" class="img-responsive">
                                        <a href="pistl.php?ID=<?php echo $row["id"]; ?>" class="text-info"><?php echo $row["title"]; ?></a>
                                    </div>
                                </form>
                            </div>
                            <?php
                            ...
                            <form method="post" action="meele.php?action=add&id=<?php echo $row["id"]; ?>">
                            <?php
                            ?>
                                <div class="pistol1">
                                    <img src="<?php echo $row["image"]; ?>" class="img-responsive">
